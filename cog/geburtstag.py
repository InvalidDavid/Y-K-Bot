...
class Geburtstag(commands.Cog):
    ...
    async def on_ready(self):
        ...
        if not self.scheduler.get_job("birthday_check"):
    ...
    def cog_unload(self):
        try:
            ...
            print(f"[Geburtstag] Scheduler shutdown error: {e}")

        if self.db.verbindung:
    ...
    async def check_geburtstag(self):
        ...
        for server in self.bot.guilds:
            ...
            if mentions:
                if len(mentions) == 1:
                    text = f"## Alles Gute zum Geburtstag! 🎉\nHeute hat {mentions[0]} Geburtstag! 🥳🎂\nWir wünschen dir einen fantastischen Tag! || @everyone ||"
                else:
                    text = f"## Alles Gute zum Geburtstag! 🎉\nHeute haben {', '.join(mentions)} Geburtstag! 🥳🎂\nWir wünschen euch fantastische Tage! || @everyone ||"
                file = discord.File(fp="Data/geburtstag.gif", filename="geburtstag.gif")
                allowed_mentions = discord.AllowedMentions(everyone=True, roles=False, users=True)
                await kanal.send(text, file=file, allowed_mentions=allowed_mentions)
    ...
    geburtstag = SlashCommandGroup("geburtstag", "Geburtstags Befehle", guild_ids=[1144592659592261724])
    admin = geburtstag.create_subgroup("admin", "Administrator", guild_ids=[1144592659592261724])
